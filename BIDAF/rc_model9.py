...
from layers.match_layer import AttentionFlowMatchLayer,MatchLSTMLayer,AttentionFlowMatchLayer3,AttentionFlowMatchLayer5
from layers.pointer_net import PointerNetDecoder
from layers.ops import *
from optimizer import *
from layers.basic_rnn import rnn,cudnn_gru
import re
...
class RCModel(object):
    ...
    def _fuse(self):
        ...
        with tf.variable_scope('fusion'):
            rnn = cudnn_gru(num_layers=1, num_units=self.hidden_size, batch_size=tf.shape(self.match_p_encodes)[0], input_size=self.match_p_encodes.get_shape().as_list()[-1], keep_prob=1-self.dropout)
            self.fuse_p_encodes = rnn(self.match_p_encodes, seq_len=self.p_length)
        with tf.variable_scope('fusion2'):
            ...
            rnn = cudnn_gru(num_layers=1, num_units=self.hidden_size, batch_size=tf.shape(gate_output)[0],
                      input_size=gate_output.get_shape().as_list()[-1], keep_prob=1 - self.dropout)
            self.fuse_p_encodes2 = rnn(gate_output, seq_len=self.p_length)
    def _decode(self):
        """
        ...
        with tf.variable_scope('same_question_concat'):
            ...
            no_dup_question_encodes = tf.reshape(
                self.sep_q_encodes,
                [batch_size, -1, tf.shape(self.sep_q_encodes)[1], 6 * self.hidden_size]
            )[0:, 0, 0:, -2*self.hidden_size:]
    ...
    def _create_train_op(self):
        ...
        if self.optim_type == 'adagrad':
            ...
            self.optimizer = tf.train.AdamOptimizer(learning_rate=self.lr, epsilon=1e-6)
            ...
            self.optimizer = AdamWOptimizer(learning_rate=self.lr, weight_decay=0.01, beta1=0.9,beta2=0.999, epsilon=1e-6)
        elif self.optim_type == "bert":
            self.optimizer = AdamWeightDecayOptimizer(learning_rate=self.lr, weight_decay_rate=0.01, beta_1=0.9,
        ...
        if self.clip_weight:
            ...
            if self.optim_type == "bert":
                new_global_step = self.global_step + 1
                train_op = tf.group(train_op, [self.global_step.assign(new_global_step)])
            self.train_op = train_op
        else:
            self.train_op = self.optimizer.minimize(self.loss, global_step=self.global_step)
    def train(self, data, epochs, batch_size, save_dir, save_prefix,dropout=0.0, evaluate=True):
        ...
        for epoch in range(1, epochs + 1):
            ...
            lr_decay = 0.95 ** max(epoch - 3, 0)
            self.sess.run(tf.assign(self.lr, tf.constant(self.learning_rate * lr_decay, dtype=tf.float32)))
            self.logger.info('learning rate deacy rate{}'.format(lr_decay))
