...
from layers.match_layer import AttentionFlowMatchLayer,MatchLSTMLayer
...
class RCModel(object):
    ...
    def _fuse(self):
        with tf.variable_scope('self-attention'):
            self.context2context,_= bilstm_layer(self.match_p_encodes, self.p_length, self.hidden_size)

            if self.use_dropout:
                self.context2context = tf.nn.dropout(self.context2context, 1 - self.dropout)

            sim_matrix = tf.matmul(self.context2context, self.context2context, transpose_b=True)
            sim_matrix /= self.hidden_size ** 0.5
            context2context_attn = tf.matmul(tf.nn.softmax(sim_matrix, -1), self.context2context)
            self.fuse_p_encodes = self.match_p_encodes + tf.nn.dropout(tf.layers.dense(
                tf.concat([self.context2context, context2context_attn, self.context2context * context2context_attn],
                          -1), self.hidden_size * 2, activation=tf.nn.relu), 1 - self.dropout)
    def _decode(self):
