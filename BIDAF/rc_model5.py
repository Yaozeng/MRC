...
class RCModel(object):
    ...
    def _match(self):
        ...
        if self.algo == 'MLSTM':
            ...
            match_layer = AttentionFlowMatchLayer3(self.hidden_size)
        else:
    ...
    def _decode(self):
        ...
        with tf.variable_scope('same_question_concat'):
            ...
            d_k = concat_passage_encodes.get_shape().as_list()[-1]
            self_attention = tf.matmul(concat_passage_encodes, concat_passage_encodes, transpose_b=True)
            self_attention/=d_k
            context2contex_attn = tf.matmul(tf.nn.softmax(self_attention, -1), concat_passage_encodes)
            concat_passage_encodes = tf.contrib.layers.layer_norm(concat_passage_encodes+context2contex_attn)
    ...
    def train(self, data, epochs, batch_size, save_dir, save_prefix,
        ...
        def _train_epoch(train_batches, dropout, merged, train_writer, sess, evaluate):
            ...
            for bitx, batch in enumerate(train_batches, 1):
                ...
                _, loss,summary,total_step = sess.run([self.train_op, self.loss,merged,self.global_step], feed_dict)
