...
class RCModel(object):
    ...
    def _fuse(self):
        with tf.variable_scope('self-attention'):
            ...
            Q = tf.layers.dense(self.fuse_p_encodes, 2*self.hidden_size, use_bias=False)  # (N, T_q, d_model)
            K = tf.layers.dense(self.fuse_p_encodes, 2*self.hidden_size, use_bias=False)  # (N, T_k, d_model)
            V = tf.layers.dense(self.fuse_p_encodes, 2*self.hidden_size, use_bias=False)  # (N, T_k, d_model)
            # Split and concat
            Q_ = tf.concat(tf.split(Q, 4, axis=2), axis=0)  # (h*N, T_q, d_model/h)
            K_ = tf.concat(tf.split(K, 4, axis=2), axis=0)  # (h*N, T_k, d_model/h)
            V_ = tf.concat(tf.split(V, 4, axis=2), axis=0)  # (h*N, T_k, d_model/h)
            ...
            context2context_attn = tf.concat(tf.split(context2context_attn, 4, axis=0), axis=2)  # (N, T_q, d_model)

            self.residual_match = self.match_p_encodes + tf.nn.dropout(tf.layers.dense(tf.concat([self.fuse_p_encodes,context2context_attn,self.fuse_p_encodes*context2context_attn], -1), self.hidden_size * 2, activation=tf.nn.relu), 1 - self.dropout)
