...
from layers.match_layer import AttentionFlowMatchLayer,MatchLSTMLayer,AttentionFlowMatchLayer3,AttentionFlowMatchLayer4,AttentionFlowMatchLayer6
...
class RCModel(object):
    ...
    def _match(self):
        ...
        if self.algo == 'MLSTM':
            ...
            match_layer = AttentionFlowMatchLayer6(self.hidden_size)
        else:
    ...
    def _fuse(self):
        ...
        with tf.variable_scope('fusion2'):
            ...
            context2contex_attn = tf.matmul(tf.nn.softmax(sim_matrix, -1), self.fuse_p_encodes)
            concat_outputs=tf.concat([self.fuse_p_encodes,context2contex_attn],-1)
            self.fuse_p_encodes2, _ = rnn('bi-lstm', concat_outputs, self.p_length,self.hidden_size, layer_num=1)
            self.fuse_p_encodes3=tf.contrib.layers.layer_norm(self.fuse_p_encodes+self.fuse_p_encodes2)
            if self.use_dropout:
                self.fuse_p_encodes3 = tf.nn.dropout(self.fuse_p_encodes3, 1 - self.dropout)
    def _decode(self):
        """
