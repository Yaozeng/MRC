...
class AttentionFlowMatchLayer2(object):
    """
    Implements the Attention Flow layer,
    which computes Context-to-question Attention and question-to-context Attention
    """
    ...
    def match(self, passage_encodes):
        """
        Match the passage_encodes with question_encodes using Attention Flow Match algorithm
        """
        with tf.variable_scope('bidaf2'):
            d_k = passage_encodes.get_shape().as_list()[-1]
            sim_matrix = tf.matmul(passage_encodes, passage_encodes, transpose_b=True)
            sim_matrix/=d_k**0.5
            context2contex_attn = tf.matmul(tf.nn.softmax(sim_matrix, -1), passage_encodes)
            concat_outputs = tf.concat([passage_encodes, context2contex_attn], -1)
            return concat_outputs, None
class AttentionFlowMatchLayer3(object):
    """
    Implements the Attention Flow layer,
    which computes Context-to-question Attention and question-to-context Attention
    """
    ...
    def match(self, passage_encodes, question_encodes, p_length, q_length):
        ...
        with tf.variable_scope('bidaf'):
            ...
            d_k = passage_encodes.get_shape().as_list()[-1]
            sim_matrix_2 = tf.matmul(passage_encodes, passage_encodes, transpose_b=True)
            sim_matrix_2/=d_k**0.5
            context2contex_attn = tf.matmul(tf.nn.softmax(sim_matrix_2, -1), passage_encodes)
            question2context_attn = tf.tile(tf.matmul(b, passage_encodes),
                                         [1, tf.shape(passage_encodes)[1], 1])
            concat_outputs = tf.concat([passage_encodes, context2question_attn,context2contex_attn,
                                        passage_encodes * context2question_attn,
                                        passage_encodes * question2context_attn], -1)
            return concat_outputs, None
class AttentionFlowMatchLayer4(object):
    """
    Implements the Attention Flow layer,
    which computes Context-to-question Attention and question-to-context Attention
    """
    ...
    def match(self, passage_encodes, question_encodes,p_length, q_length):
        ...
        with tf.variable_scope('bidaf'):
            ...
            return concat_outputs * gate,None
class AttentionFlowMatchLayer5(object):
    """
    Implements the Attention Flow layer,
    which computes Context-to-question Attention and question-to-context Attention
    """
    ...
    def match(self, passage_encodes, question_encodes,mask_c,mask_q,keep_prob,p_length, q_length):
        """
        Match the passage_encodes with question_encodes using Attention Flow Match algorithm
        """
        with tf.variable_scope('bidaf'):
            d_inputs = dropout(passage_encodes, keep_prob=keep_prob)
            d_memory = dropout(question_encodes, keep_prob=keep_prob)
            ...
            mask_q = tf.tile(tf.expand_dims(mask_q, axis=1), [1, JX, 1])
            context2question_attn = tf.matmul(tf.nn.softmax(softmax_mask(sim_matrix,mask_q), -1), question_encodes)

            mask_c = tf.expand_dims(mask_c, axis=1)
            b = tf.nn.softmax(softmax_mask(tf.expand_dims(tf.reduce_max(sim_matrix, 2), 1),mask_c), -1)
            question2context_attn = tf.tile(tf.matmul(b, passage_encodes),
                                         [1, tf.shape(passage_encodes)[1], 1])
            ...
            dim = concat_outputs.get_shape().as_list()[-1]
            d_concat_outputs = dropout(concat_outputs, keep_prob=keep_prob)
            gate = tf.nn.sigmoid(dense(d_concat_outputs, dim, use_bias=False))
            return concat_outputs * gate,None
class AttentionFlowMatchLayer6(object):
    """
    Implements the Attention Flow layer,
    which computes Context-to-question Attention and question-to-context Attention
    """
    ...
    def match(self, passage_encodes, question_encodes,p_length, q_length):
        ...
        with tf.variable_scope('bidaf'):
            ...
            concat_outputs = tf.concat([passage_encodes, context2question_attn,
                                        ...
                                        passage_encodes * question2context_attn], -1)
            return concat_outputs,None
class AttentionFlowMatchLayer7(object):
    """
    Implements the Attention Flow layer,
    which computes Context-to-question Attention and question-to-context Attention
    """
    ...
    def match(self, passage_encodes, question_encodes,p_length, q_length):
        ...
        with tf.variable_scope('bidaf'):
            ...
            sim_matrix_2 = tf.matmul(passage_encodes, passage_encodes, transpose_b=True)
            sim_matrix_2 /= self.hidden_size ** 0.5
            context2contex_attn = tf.matmul(tf.nn.softmax(sim_matrix_2, -1), passage_encodes)
            ...
            concat_outputs = tf.concat([passage_encodes, context2question_attn,context2contex_attn,
                                        ...
                                        passage_encodes * question2context_attn], -1)
            return concat_outputs, None
class AttentionFlowMatchLayer8(object):
    """
    Implements the Attention Flow layer,
    which computes Context-to-question Attention and question-to-context Attention
    """
    ...
    def match(self, passage_encodes, question_encodes,mask_c,mask_q,p_length, q_length):
        """
        Match the passage_encodes with question_encodes using Attention Flow Match algorithm
        """
        with tf.variable_scope('bidaf'):
