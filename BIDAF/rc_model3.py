...
from layers.match_layer import AttentionFlowMatchLayer,MatchLSTMLayer,AttentionFlowMatchLayer2
...
class RCModel(object):
    ...
    def _embed(self):
        ...
        with tf.variable_scope('word_embedding'):
            ...
            #self.p_emb = highway(p_emb, size=self.hidden_size, scope="highway", dropout=self.dropout, reuse=None)
            #self.q_emb = highway(q_emb, size=self.hidden_size, scope="highway", dropout=self.dropout, reuse=True)
    ...
    def _fuse(self):
        ...
        with tf.variable_scope('fusion'):
            self.fuse_passage_question_encodes, _ = rnn('bi-lstm', self.match_p_encodes, self.p_length,self.hidden_size, layer_num=1)

        match_layer2 = AttentionFlowMatchLayer2(self.hidden_size)
        self.passage_attention, _ = match_layer2.match(self.fuse_passage_question_encodes)

        with tf.variable_scope('self-attention'):
            self.fuse_p_encodes, _ = rnn('bi-lstm', self.passage_attention, self.p_length,self.hidden_size, layer_num=1)
            self.fuse_p_encodes = ln(self.fuse_p_encodes+self.fuse_passage_question_encodes)
        if self.use_dropout:
