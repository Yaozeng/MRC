...
from layers.match_layer import AttentionFlowMatchLayer,MatchLSTMLayer,AttentionFlowMatchLayer3
...
class RCModel(object):
    ...
    def _encode(self):
        ...
        with tf.variable_scope('passage_encoding'):
            self.sep_p_encodes, _ = bilstm_layer('bi-lstm', self.p_emb, self.p_length, self.hidden_size)
        with tf.variable_scope('question_encoding'):
            self.sep_q_encodes, _ = bilstm_layer('bi-lstm', self.q_emb, self.q_length, self.hidden_size)
        if self.use_dropout:
