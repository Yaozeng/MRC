...
class Model(object):
    ...
    def _fuse(self):
        ...
        with tf.variable_scope("Context_to_Query_Attention_Layer"):
            ...
            #S = optimized_trilinear_for_attention([self.c_embed_encoding, self.q_embed_encoding], self.max_p_len, self.max_q_len,input_keep_prob=1.0 - self.dropout)
        ...
        with tf.variable_scope("Model_Encoder_Layer"):
            ...
            for i, item in enumerate(self.enc):
                self.enc[i] = tf.reshape(self.enc[i],
                                         [batch_size, -1, self.enc[i].get_shape()[-1]])
    def _decode(self):
        batch_size = tf.shape(self.start_label)[0]
        ...
        self.logits = [mask_logits(start_logits, mask=tf.reshape(self.c_mask, [batch_size, -1])),
                       mask_logits(end_logits, mask=tf.reshape(self.c_mask, [batch_size, -1]))]
    ...
    def _params(self):
        return (self.max_p_len, self.max_q_len,self.config.hidden_size,self.config.head_size)

    def train(self, data, epochs, batch_size, save_dir, save_prefix,
              dropout=0.0, evaluate=True):

        def _train_epoch(train_batches, dropout, merged, train_writer, sess, evaluate, max_rouge_l):
            ...
            for bitx, batch in enumerate(train_batches, 1):
                ...
                if evaluate and total_step % 5000 == 0:
    ...
    def evaluate(self, eval_batches, result_dir=None, result_prefix=None, save_full_info=False):
        ...
        for b_itx, batch in enumerate(eval_batches):
            ...
            start_probs, end_probs= self.sess.run([self.logits1, self.logits2], feed_dict)
            #total_loss += loss * len(batch['raw_data'])
            #total_num += len(batch['raw_data'])
